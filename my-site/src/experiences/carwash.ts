import {
  ApiError, createReservation, getCarWashCatalog, getCarWashQuote, getCarWashAvailability,
  type CarWashCatalog, type CarWashQuote, type ReservationPayload,
} from '../api';
...
export function initCarWashExperience(config: PublicTenantConfig): void {
  ...
  if (!form) return;
  const el = <T extends HTMLElement>(key: string) => form.querySelector<T>(`[data-carwash="${key}"]`)!;
  const steps = [...form.querySelectorAll<HTMLFieldSetElement>('[data-carwash-step]')];
  ...
  const selection = () => ({ vehicleCategoryId: categoryId, serviceIds: [...selected] });
  ...
  function updateSummary() {
    ...
    summary.textContent = quote
      ...
      : selected.size ? `Выбрано услуг: ${selected.size}. Стоимость и время уточним на следующем шаге.` : '';
  }
  function invalidate() {
    revision++;
    quote = null;
  ...
  function showStep(index: number, focus = true) {
    ...
    el<HTMLElement>('progress').textContent = `Шаг ${index + 1} из ${steps.length}`;
    back.hidden = index === 0;
  ...
  function setBusy(value: boolean) {
    ...
    steps.forEach(step => { step.disabled = value; });
    back.disabled = value;
    submit.disabled = value || !catalog;
    submit.textContent = value ? 'Загрузка…' : currentStep === steps.length - 1 ? 'Записаться' : 'Далее';
  }
  function option(text: string, active: boolean, click: () => void) {
    ...
    button.addEventListener('click', () => { if (!busy) click(); });
    return button;
  }
  function renderCategories() {
    categoryOptions.replaceChildren(...(catalog?.categories ?? []).map(category =>
  ...
  function renderServices() {
    const included = new Set(catalog?.packageItems
      .filter(item => selected.has(item.packageServiceId)).map(item => item.includedServiceId));
    serviceOptions.replaceChildren(...(catalog?.services ?? [])
        ...
        const button = option(
          ...
          selected.has(service.id) || isIncluded, () => {
            if (selected.has(service.id)) selected.delete(service.id);
              ...
              catalog?.packageItems.filter(item => item.packageServiceId === service.id)
  ...
  async function loadTimes() {
    ...
    try {
      ...
      el<HTMLElement>('slot-status').textContent = slots.length
        ? 'Время указано по Казахстану.' : 'Свободного времени нет. Нажмите «Назад» и выберите другую дату.';
      times.replaceChildren(...slots.map(slot => {
        const button = option(slot.slice(11) + (slot.slice(0, 10) !== date.value ? ' (+1 день)' : ''), false, () => {
      ...
      if (version !== revision) return;
      el<HTMLElement>('slot-status').textContent = 'Не удалось проверить время. Нажмите «Назад» и попробуйте снова.';
      throw error;
    }
  }
  function validate(index: number): boolean {
    ...
    let error = '';
    if (step.dataset.carwashStep === 'category' && !categoryId) error = 'Выберите категорию автомобиля.';
    if (step.dataset.carwashStep === 'service' && !selected.size) error = 'Выберите хотя бы одну услугу.';
    if (step.dataset.carwashStep === 'time' && (!slots.includes(scheduledAt.value) ||
      new Date(`${scheduledAt.value}:00+05:00`).getTime() < Date.now() + 5 * 60_000)) error = 'Выберите доступное время записи.';
    const input = step.querySelector<HTMLInputElement>('input:not([type="hidden"]):not([type="checkbox"])');
    input?.setCustomValidity('');
    if (step.dataset.carwashStep === 'plate' && !input?.value.trim()) input?.setCustomValidity('Укажите гос. номер.');
    if (step.dataset.carwashStep === 'phone') {
      const digits = input?.value.replace(/\D/g, '') ?? '';
      if (digits.length < 10 || digits.length > 15) input?.setCustomValidity('Укажите полный номер телефона с кодом страны.');
    }
  ...
  back.addEventListener('click', () => { if (!busy && currentStep > 0) showStep(currentStep - 1); });
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      ...
      const payload: ReservationPayload = {
        ...
        tablesId: '', section: '', remindBeforeHour: false,
      };
      let result;
      try { result = await createReservation(payload); }
      catch (error) {
        if (!(error instanceof ApiError) || error.code !== 'EXISTING_RESERVATION') throw error;
        if (!window.confirm(`У вас уже есть запись ${error.existing?.scheduledAt.replace('T', ' ') ?? ''}. Заменить её?`)) {
      ...
      if (error instanceof ApiError && error.code === 'SLOT_TAKEN') {
        ...
        try { await loadTimes(); } catch { /* Preserve booking conflict message. */ }
      }
      status(error instanceof Error ? error.message : 'Не удалось выполнить запрос. Попробуйте снова.', true);
    } finally { setBusy(false); }
  });
  showStep(0, false);
  setBusy(true);
  void getCarWashCatalog().then(result => {
    catalog = result;
    if (!result.categories.length || !result.services.length) throw new Error('Онлайн-запись пока недоступна: услуги не настроены.');
    renderCategories();
    renderServices();
    const list = document.querySelector<HTMLElement>('[data-carwash="services"]');
    const unique = [...new Map(result.services.map(service => [service.id, service])).values()];
    list?.replaceChildren(...unique.map(service => {
    ...
    catalog = null;
    status(error instanceof Error ? error.message : 'Не удалось загрузить услуги. Обновите страницу.', true);
  }).finally(() => setBusy(false));

  for (const [key, value] of [['start-time', config.bookingTime.startTime], ['end-time', config.bookingTime.endTime]]) {
  ...
  toggle?.addEventListener('click', () => toggle.setAttribute('aria-expanded', String(nav?.classList.toggle('nav-open') ?? false)));
