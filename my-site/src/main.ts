...
bookBtn?.addEventListener("click", () => {
  const reservationSection =
    document.querySelector<HTMLElement>(".reservation");
  ...
  try {
    ...
    if (ratingEl)
      ratingEl.textContent = Number.isFinite(rating)
        ...
        : "0.0";
    if (reviewsEl)
      reviewsEl.textContent = Number.isFinite(reviewCount)
        ? String(reviewCount)
        : "0";
  } catch (err) {
...
const openTablePickerBtn = document.getElementById(
  "openTablePicker",
) as HTMLButtonElement | null;
const selectedTableBadge = document.getElementById(
  "selectedTableBadge",
) as HTMLElement | null;

const tablePlaceholder = document.querySelector<HTMLElement>(
  ".table-picker-placeholder",
);
...
const mobileCloseModalBtn = document.getElementById(
  "mobileCloseTableModal",
) as HTMLButtonElement | null;
const toggleSidebarBtn = document.getElementById(
  "toggleSidebarBtn",
) as HTMLButtonElement | null;
const tableSidebar = document.querySelector<HTMLElement>(".table-sidebar");
const confirmTableBtn = document.getElementById(
  "confirmTableBtn",
) as HTMLButtonElement | null;
...
const successModalText = document.getElementById(
  "successModalText",
) as HTMLElement | null;
const successModalCloseBtn = document.getElementById(
  "successModalCloseBtn",
) as HTMLButtonElement | null;
const selectedSummary = document.getElementById(
  "selectedSummary",
) as HTMLElement | null;
const selectedTablesList = document.getElementById(
  "selectedTablesList",
) as HTMLElement | null;
...
function centerActiveScene() {
  ...
  const activeScene = document.querySelector<HTMLElement>(
    ".floor-plan-scene.active",
  );
  if (!activeScene) {
    centerCanvas();
    return;
...
const tablePopupIcon = document.getElementById(
  "tablePopupIcon",
) as HTMLElement | null;
const tablePopupTitle = document.getElementById(
  "tablePopupTitle",
) as HTMLElement | null;
const tablePopupText = document.getElementById(
  "tablePopupText",
) as HTMLElement | null;
const tablePopupActions = document.getElementById(
  "tablePopupActions",
) as HTMLElement | null;
...
function showBlockedPopup(marker?: HTMLButtonElement) {
  ...
  if (nextReservationHours !== null) {
    ...
    tablePopupText.textContent =
      "Этот столик уже забронирован другими гостями. Выберите другой столик или другое время.";
  }
...
const tableMarkers =
  document.querySelectorAll<HTMLButtonElement>(".table-marker");

async function refreshTableStatuses() {
  let tables: TableAvailability[];
  try {
    const datetimeValue = (
      document.getElementById("datetime") as HTMLInputElement | null
    )?.value;
  ...
  tableMarkers.forEach((marker) => {
    ...
    if (
      status === "limited" &&
      info.nextReservationHours != null &&
      info.nextReservationHours < THRESHOLD_HOURS
    ) {
      status = "occupied";
    }
...
confirmTableBtn?.addEventListener("click", () => {
  ...
  const labels = markers.map((m) =>
    m.classList.contains("vip-table")
      ? `VIP №${m.dataset.id}`
      : `№${m.dataset.id}`,
  );
...
const submitButton =
  reservationForm?.querySelector<HTMLButtonElement>('button[type="submit"]') ??
  null;
const reservationStatus = document.getElementById(
  "reservationStatus",
  ...
  if (!isOpen) {
    if (showMessage) {
      setReservationStatus(
        "Ресторан работает с 12:00 до 04:00. Выберите время в рабочем окне.",
        "error",
      );
      datetimeInput.focus();
    }
...
function getActiveFloorSection(): string {
  const activeTab =
    document.querySelector<HTMLButtonElement>(".floor-tab.active");
...
reservationForm?.addEventListener("submit", async (e) => {
  ...
  const selectedIds = Array.from(selectedTables).map(
    (marker) => marker.dataset.id ?? "",
  );
  ...
  if (
    ...
    setReservationStatus(
      "Пожалуйста, заполните имя, телефон, время и выберите столик.",
      "error",
    );
    return;
  }
  ...
  if (!/^\+7\d{10}$/.test(payload.customerPhone)) {
    setReservationStatus(
      "Пожалуйста, укажите номер телефона полностью: +7 700 (000) 00 00.",
      "error",
    );
    phoneInput?.focus();
    return;
  ...
  try {
    ...
    const successMessage =
      response.message || response.reservationId
